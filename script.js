...
async function startAutomatedRecording(type) {
    try {
        ...
        const reqFps = parseInt(document.getElementById('custom-fps').value) || 120; // Default bumped to 120
        ...
        // STRICTLY MP4 Logic. Removed WebM completely.
        let mimeType = 'video/mp4;codecs=h264';
        let fileExt = formatSelect; // Supports 'mp4' or the 'mov' placeholder

        // Enforce MP4 container natively if perfectly matching h264 codec throws a warning
        if (!MediaRecorder.isTypeSupported(mimeType)) {
            console.warn("video/mp4;codecs=h264 natively unsupported by this browser. Forcing generic MP4 container.");
            mimeType = 'video/mp4'; 
        }

        const options = { mimeType: mimeType, videoBitsPerSecond: 100000000 };

        // Setup Capture Stream perfectly tied to native 120 FPS
        ...
        // Frame Capture Loop (DOM to Canvas) natively syncing with device's 120Hz via requestAnimationFrame
